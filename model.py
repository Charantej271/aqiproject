...
MODEL_METRICS = {}  #for registering best model to mlflow automatically
MODEL_OBJECTS = {}  #for registering best model to mlflow automatically
...
MODEL_METRICS["LinearRegression"] = lr_rmse
MODEL_OBJECTS["LinearRegression"] = lr_model
...
MODEL_METRICS["DecisionTree"] = dt_rmse
MODEL_OBJECTS["DecisionTree"] = dt_model
...
# ---------------- EVALUATION (CITY-SPECIFIC) ----------------

rf_rmse = np.sqrt(mean_squared_error(y_city_test, y_city_pred_rf))
rf_mae  = mean_absolute_error(y_city_test, y_city_pred_rf)

print("\nRandom Forest Performance (City-Specific)")
print("RMSE:", rf_rmse)
print("MAE :", rf_mae)

#rf model metrics and objects to be tracked in mlflow
MODEL_METRICS["RandomForest"] = rf_rmse
MODEL_OBJECTS["RandomForest"] = rf_model
...
MODEL_METRICS["XGBoost"] = xgb_rmse
...
best_model_rmse = MODEL_METRICS[best_model_name]
...
#GEOGRAPHICAL (CITY) INFLUENCE — CORRECT HANDLING
...
def save_model_artifacts(model, model_name, scaler, feature_columns, base_dir):
    """
    Save model, scaler, and feature columns with dynamic names
    """
    os.makedirs(base_dir, exist_ok=True)

    model_path = os.path.join(base_dir, f"{model_name}_model.pkl")
    scaler_path = os.path.join(base_dir, f"{model_name}_scaler.pkl")
    features_path = os.path.join(base_dir, f"{model_name}_features.pkl")

    joblib.dump(model, model_path)
    joblib.dump(scaler, scaler_path)
    joblib.dump(feature_columns, features_path)

    print(f"Saved artifacts for model: {model_name}")
...
save_model_artifacts(
    model=best_model,
    model_name=best_model_name.lower(),   # e.g. linearregression / randomforest / xgboost
    scaler=scaler,
...
print(" Model artifacts saved in /artifacts")
